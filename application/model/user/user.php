...
class User {
	...
	public function add($email, $login, $pass, $pass2, $name) {
		...
		if (empty($email) || empty($login) || empty($pass) || empty($pass2)) {
			...
			return new Response(4, "danger", $this->_lang->get("user", "add.emailInvalid"));
		} elseif (!StringCheckers::isValidLogin($login)) {
			return new Response(5, "danger", $this->_lang->get("user", "add.loginInvalid"));
		} elseif (!$this->check("email", $email)) {
			return new Response(6, "danger", $this->_lang->get("user", "add.emailNotAvailable"));
		} elseif (!$this->check("login", $login)) {
			return new Response(7, "danger", $this->_lang->get("user", "add.loginNotAvailable"));
		} elseif ($pass != $pass2) {
			return new Response(8, "danger", $this->_lang->get("user", "add.passwordsNotMatch"));
		} elseif (!empty($name) && !StringCheckers::isValidName($name)) {
			return new Response(9, "danger", $this->_lang->get("user", "add.nameInvalid"));
		}
		...
		return new Response(0, "success", $this->_lang->get("user", "success"));
	}
